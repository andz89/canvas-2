...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  let uploadedImages = [];
  // Show modal after saving the PDF
  function showModal() {
    document.getElementById("pdfReadyModal").classList.remove("hidden");
  }
  function showModalSuccessfull() {
    document.getElementById("successful-modal").classList.remove("hidden");
  }
  ...
  exportBtn.addEventListener("click", async function () {
    ...
    const cellWidth = 380; //350;
    const cellHeight = 280; // 250
    const spacingX = 0;
    const spacingY = 10;
    const marginX = (pageWidth - (2 * cellWidth + spacingX)) / 2;
    const marginY = (pageHeight - (2 * cellHeight + spacingY)) / 2;
    document.querySelector("#pdfReadyModal").classList.add("hidden");
    showModalWaiting();
    ...
    showModalSuccessfull();
    document.getElementById("closeModalBtn").addEventListener("click", () => {
      document.querySelector("#successful-modal").classList.add("hidden");
    });
  });
});
